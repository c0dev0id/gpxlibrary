    ...
    async function uploadGpxFiles(files, folderId = null) {
        ...
        for (const file of files) {
            try {
                ...
                // Re-parse normalized content to get structured data
                const normalizedData = GPXParser.parse(normalizedContent);
                
                // Store routes
                if (normalizedData.routes) {
                    for (const route of normalizedData.routes) {
                ...
                // Store waypoints
                if (normalizedData.waypoints) {
                    for (const waypoint of normalizedData.waypoints) {
                        await Database.execute(
